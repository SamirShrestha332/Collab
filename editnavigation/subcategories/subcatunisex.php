...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_product'])) {
    ...
    $category_id = 3; // Replace 1 with the actual category ID from the categories table
    insert_product($conn, "sub_categories_unisex", $product_name, $category_id);
}

// Handle insertion of a new product
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_product'])) {
    $product_name = $_POST['product_name'];
    insert_product($conn, "sub_categories_unisex", $product_name);
}
...
$stmt = $conn->prepare("UPDATE sub_categories_unisex SET is_hidden = 1 WHERE ID = ?");
...
$sql = "SELECT * FROM sub_categories_unisex WHERE is_hidden = 0";
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>" . $row["Name"] . "</td>";
                        echo "<td>
                                <a href='editunisex.php?id=" . $row["ID"] . "'>Edit</a> |
